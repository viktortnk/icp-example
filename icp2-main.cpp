...
vector<int> NN(PointCloudT::Ptr target, PointCloudT::Ptr source, Eigen::Matrix4d initTransform, double dist) {
    ...
    // function which returns a vector of target indicies that correspond to each source index inorder.
    ...
    for (const auto &sourcePoint: transformSource->points) {
        vector<int> pointIdxRadiusSearch;
        vector<float> pointRadiusSquaredDistance;
        int res = kdtree.radiusSearch(sourcePoint, dist, pointIdxRadiusSearch, pointRadiusSquaredDistance);
        if (res == -1) {
...
vector<Pair> PairPoints(vector<int> associations, PointCloudT::Ptr target, PointCloudT::Ptr source, bool render, pcl::visualization::PCLVisualizer::Ptr &viewer) {
    ...
    for (int i = 0; i < source->size(); ++i) {
        auto s = (*source)[i];
        int assocIdx = associations[i];
        if (assocIdx > 0) {
            auto t = (*target)[assocIdx];

            Point p1 = Point{s.x, s.y, s.z};
            Point p2 = Point{t.x, t.y, t.z};
            Pair p{p1, p2};
            pairs.push_back(p);

            if (render) {
                viewer->removeShape(to_string(i));
                renderRay(viewer, p1, p2, to_string(i), Color(0, 1, 0));
            }
        }
    }
